\subsection{Sprint 1: Setup, Research, and Initial Architecture}\label{sec:sprint1}
...
Responsibilities followed these workstreams. Marco established the Jira structure and led the research on LLM judges, judge panels, and their biases. Julius created the repositories and initial application, implemented the first chatbot interface, prepared the sprint documentation, and coordinated the sprint. Mohsin investigated inference-token access and external providers. Sebastian selected and documented the local Ollama setup and researched multi-agent routing. Gáspár defined the initial metrics and researched prompt optimization, DSPy, and self-improving pipelines. The architecture, MVP definition, repository strategy, and Sprint Review were developed collaboratively.
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{assets/sprints/sprint1-backlog.png}}
\caption{Sprint 1 backlog in Jira with story-point estimates and completion status.}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{assets/sprints/sprint1-production-path.png}}
\caption{Initial Sprint 1 production path from a user query through the Router and selected specialist agent to the answer}
...
\caption{Sprint 1 burndown chart.}\label{fig:sprint1-burndown}
\end{figure}
...
\captionof{figure}{Sprint 1 retrospective feedback grouped into Continue, Change, New Ideas, and Appreciate.}
...
\captionof{figure}{Action items agreed in the Sprint 1 retrospective, including owners and deadlines.}
\label{fig:sprint1-retrospective-actions}
\end{center}

\subsubsection{Sprint Conclusion and Outlook}\label{sec:sprint1-conclusion}

Sprint 1 moved the project from a broad brief to a structured development environment and a concrete, research-informed architecture. The team established its repositories and workflow, delivered the first showcase-application increment, resolved the immediate model-access blocker, and defined an evaluation perspective spanning quality, latency, and cost. It also proposed how live interaction, trace collection, evaluation, and optimization could be separated.

The sprint created the conditions for experimentation rather than the completed pipeline itself. Sprint 2 would therefore define the concrete multi-agent scenario and specialist agents, establish the evaluation data, implement the first Router and judge components, and begin testing the proposed architecture in working code.
