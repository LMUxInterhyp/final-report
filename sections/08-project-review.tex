...
A second limitation is that the self-improvement loop relies on a single judge. While the judge currently achieves 99\% agreement with the ground truth, this does not mean it is guaranteed to be error-free. The concern is structural: because every correction to the dispatcher comes from this one component, its mistakes have nowhere to be caught. Any error the judge makes would feed straight into the Stage 1 store and the optimizer and shape the dispatcher from there.
