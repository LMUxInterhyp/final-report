...
The optimizer inherits related constraints. It depends entirely on the judge's verdict file as ground truth, so a systematic judge bias would misdirect the whole search rather than affect an isolated case, and a small or unevenly distributed verdict file risks fitting that file rather than producing a prompt that generalizes. The method itself, adopted from OPRO, is known to depend on the starting prompt and offers no guarantee that each round improves on the last (Yang et al., 2024).
